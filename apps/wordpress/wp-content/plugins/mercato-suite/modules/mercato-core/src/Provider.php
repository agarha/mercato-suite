...
final class Provider extends ServiceProvider
{
    ...
    public function demoFeatures(): \WP_REST_Response
    {
        global $wpdb;
        $prefix = $wpdb->prefix;
        $query = static fn (string $sql): array => $wpdb->get_results($sql, ARRAY_A) ?: [];
        $scalar = static fn (string $sql): int => (int) $wpdb->get_var($sql);
        ...
        return new \WP_REST_Response([
            'features' => $features,
            'recent_vendors' => $query("SELECT v.vendor_id, v.business_name, v.store_slug, v.status, COALESCE(k.status, 'not_started') AS kyc_status, v.stripe_account_id FROM `{$prefix}mercato_vendors` v LEFT JOIN `{$prefix}mercato_kyc_cases` k ON k.vendor_id = v.vendor_id AND k.tenant_id = v.tenant_id ORDER BY v.vendor_id DESC LIMIT 8"),
            ...
            'recent_payouts' => $query("SELECT b.batch_id, b.status, b.total_minor, COUNT(i.payout_item_id) AS item_count, b.created_at FROM `{$prefix}mercato_payout_batches` b LEFT JOIN `{$prefix}mercato_payout_items` i ON i.batch_id = b.batch_id AND i.tenant_id = b.tenant_id GROUP BY b.batch_id, b.status, b.total_minor, b.created_at ORDER BY b.batch_id DESC LIMIT 8"),
